Feature: Web Automation
  ...
  @Run
  Scenario: Sauce Demo app - User input invalid username and password - Login failed
    When QA go to website "https://www.saucedemo.com/"
    And QA input username "invalid_username" and password "invalid_password"
    And QA click Submit button on the login page
    And QA verify that error message "Epic sadface: Username and password do not match any user in this service" is displayed
    And QA click close error button on the login page
    And QA verify that component with data-test "error" is NOT displayed on the page
